...
<div class="product-section container">
    ...
    <div class="product-section-information">
        <div class="product-section-first-row">
            ...
            <form class="heart-item-form" action="{{ route('wishlist.destroy', $product->getCartRowId('wishlist')) }}"
                method="post">
                @csrf
                @method('DELETE')

                <button type="submit" class="button-icon" title="Remove from Wishlist">
                    <span class="remove-from-wishlist">Supprimer de la liste de souhaits</span>&nbsp;<i
                        class="fas fa-heart solid-heart"></i>
                </button>
            </form>
            @else
            ...
            <form class="heart-item-form" action="{{ route('wishlist.store') }}" method="post">
                ...
                <button type="submit" class="button-icon" title="Add to Wishlist">
                    <span class="add-to-wishlist">Ajouter à la liste de souhaits</span>&nbsp;<i
        ...
        @if ($product->isInCart())
        <div class="alert-success">Deja dans le panier</div>
        @else
        <form action="{{ route('cart.store') }}" method="post">
            ...
            <input type="hidden" name="name" value="{{ $product->name }}">
            <input type="hidden" name="price" value="{{ $product->price }}">
            <button class="button button-plain add-to-cart-btn" type="submit">Ajouter au panier <i
